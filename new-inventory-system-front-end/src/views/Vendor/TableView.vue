<template>
    ...
    <template v-slot:table-body>
      <tr v-for="(item, index) in data.data" :key="item.id">
        ...
        <td :class="{'py-3 pl-2 bg-gray-400': isOdd(index), 'py-3 pl-2 bg-gray-200': !isOdd(index)}">{{ item.name }}</td>
        <td :class="{'py-3 pl-2 bg-gray-400': isOdd(index), 'py-3 pl-2 bg-gray-200': !isOdd(index)}">{{ item.contact_email }}</td>
    ...
    const headers= ref(  [
          { field: 'id', label: 'Id' },
            ...
            { field: 'name', label: 'name' },
            { field: 'contact_email', label: 'contact_email' },
            { field: 'action', label: 'Action' }
          ]);
          const createRoutName= ref('/vendors/create');
    
          const data = computed(() =>store.state.vendors) 
    ...
    store.dispatch('deletevendor', item)
   .then(res => {
 if (res.status === 200) { // check if deletion is successful
    // show success message to user
    store.dispatch('getvendors')
    store.commit('showToast', {message:'vendor has been deleted successfully.'})
  }
}).catch(error => {
  if (error.response.status === 403) { // check if user is unauthorized
    ...
    store.commit('showToast', {message:'An error occurred while deleting the vendor.',type:'danger'})
  }
  // show error message to user
})
      }
